...
class DragHandler:
    ...
    def handle_drag_start(self, widget, event):
        """Handle drag start with initialization check"""
        if not hasattr(self.main_window, '_drag_initialized') or not self.main_window._drag_initialized:
            print("⌛ Drag system not ready")
            return False
